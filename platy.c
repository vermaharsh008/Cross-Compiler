...
int main(int argc, char ** argv) {
	...
	/*Name of the output file with .c extension*/
	int length;
	...
	outputFile = (char *)malloc(length+1);
	strcpy(outputFile, argv[1]);
	/*Checks if the character read is . if then it adds c(extension) and a null terminator at the end of the outputFile*/
	...
	/*Initialing file streams with write mode*/
	c = fopen("temp.txt", "w"); 
	code = fopen(outputFile, "w");
	parser();
	/*Closing the files streams*/
	fclose(c);
	fclose(code);
	/*The C code file is removed if there are any syntax errors*/
	if (synerrno) {
		printf("\nUnable to generate C code due to syntax errors.\n");
		remove(outputFile);
	}
	/*Removing or deleting the temporary file*/
	remove("temp.txt");
	if (outputFile != NULL)
		free(outputFile);
